...
const locationText = computed(() => {
  return props.userLocation ? 'Terdekat' : 'Semua';
});
  ...
  <div class="flex flex-col gap-4 h-full">
    ...
    <div class="flex flex-col gap-4 shrink-0">
      <div class="flex flex-col gap-2">
        <h2 class="text-lg font-bold leading-tight text-[#2D2416] font-['Libre_Baskerville']">
          Jelajahi Museum
        </h2>
        <p class="text-sm text-[#6B5D4F]">
          Temukan warisan budaya Yogyakarta
        </p>
      </div>
      ...
      <div class="flex flex-col gap-3">
        <div class="relative">
          <Search class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#6B5D4F]" />
          <input
            v-model="searchQuery"
            type="text"
            placeholder="Cari museum..."
            class="w-full pl-10 pr-4 py-2.5 bg-white border border-[#6B4423]/20 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#6B4423] focus:border-transparent transition-all"
          />
        </div>
          ...
          <button
            ...
            :class="
              activeCategory === filter.value
                ...
                : 'bg-white text-[#6B5D4F] border-[#6B4423]/20 hover:border-[#6B4423]'
            "
          >
            {{ filter.label }}
          </button>
      ...
      <div class="h-px bg-[#6B4423]/10" />
    </div>

    <!-- Stats Bar -->
    <div class="bg-[#6B4423]/5 p-4 flex justify-between items-center text-xs rounded-lg shrink-0">
      <span class="font-bold text-[#2D2416]">
        <span class="text-lg text-[#6B4423]">
          {{ filteredMuseums.length }}
        </span>
        museum
      </span>
      <span class="text-[#6B5D4F] font-medium flex items-center gap-1">
    ...
    <div class="flex flex-col gap-3 flex-1 min-h-0 overflow-y-auto pr-2">
      <div
        ...
        :key="museum.id"
        class="group relative flex flex-col gap-3 p-3 bg-white border border-[#6B4423]/10 rounded-lg shadow-sm hover:shadow-md transition-all cursor-pointer hover:border-[#D4A574]"
        @click="handleMuseumClick(museum.id)"
      >
        <div class="flex gap-3">
          <div class="relative w-20 h-20 shrink-0 rounded-lg overflow-hidden bg-[#6B4423]/5 border border-[#6B4423]/10">
            <img
              :src="museum.image"
              :alt="museum.name"
          ...
          <div class="flex flex-col flex-1 min-w-0 justify-center">
            <h3 class="font-bold text-sm text-[#2D2416] leading-tight group-hover:text-[#6B4423] transition-colors line-clamp-2">
              {{ museum.name }}
            </h3>
        ...
        <p class="text-xs text-[#6B5D4F] leading-relaxed line-clamp-2 border-t border-dashed border-[#6B4423]/10 pt-2">
          {{ museum.description }}
        </p>

        <div class="flex items-center justify-between gap-2">
          <div class="flex flex-col min-w-0">
            <span class="text-[10px] font-bold text-[#2D2416] truncate">
              {{ museum.artifactCount }} artefak
            </span>
            <span class="text-[10px] text-[#6B4423] font-medium truncate">
              {{
                museum.distance
          ...
          <button
            class="shrink-0 px-3 py-1.5 rounded-lg bg-[#6B4423]/5 text-[#6B4423] text-[10px] font-bold border border-[#6B4423]/20 hover:bg-[#6B4423] hover:text-white hover:border-[#6B4423] transition-all active:scale-95"
      ...
      >
        <MapPin class="w-12 h-12 text-[#6B4423]/20 mb-2" />
        <p class="text-sm text-[#6B5D4F] font-medium">
          Tidak ada museum ditemukan
        </p>
        <p class="text-xs text-[#6B5D4F]/70 mt-1">
