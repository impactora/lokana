...
const canRedeem = (xpRequired: number) => {
  return userStore.currentXP >= xpRequired;
};

const handleRedeem = (voucherId: string, xpRequired: number) => {
  if (!canRedeem(xpRequired)) {
    alert(`XP tidak mencukupi! Anda butuh ${xpRequired} XP untuk menukar voucher ini.`);
    return;
  }
  ...
  const confirm = window.confirm(`Tukar ${xpRequired} XP dengan voucher ini?`);
  if (confirm) {
    userStore.gainXP(-xpRequired);
  ...
  <div class="h-full overflow-y-auto p-4 md:p-6">
    <div class="max-w-5xl mx-auto space-y-6">
      ...
      <div class="bg-gradient-to-r from-[#2C5F4F] to-[#3A7763] rounded-2xl p-6 text-white">
        ...
        <p class="text-white/80">Tukar XP Anda dengan voucher menarik dari partner kami!</p>
        <div class="mt-4 flex items-center gap-4">
          <div class="bg-white/20 px-4 py-2 rounded-lg">
            <span class="text-sm">XP Anda:</span>
            <span class="text-2xl font-bold ml-2">{{ userStore.currentXP }}</span>
          </div>
        </div>
      </div>
          ...
          <div class="p-4 space-y-3">
            ...
            <div class="flex items-center justify-between pt-2 border-t border-gray-100">
              <div>
                ...
                <p class="text-lg font-bold text-[#2C5F4F]">{{ voucher.xpRequired }} XP</p>
              </div>
              <button
                @click="handleRedeem(voucher.id, voucher.xpRequired)"
      ...
      <div class="bg-amber-50 border border-amber-200 rounded-xl p-4">
        ...
        <ul class="text-sm text-amber-700 space-y-1">
          <li>1. Kumpulkan XP dengan menjelajahi museum dan menjawab quiz</li>
          <li>2. Pilih voucher yang ingin ditukar</li>
          <li>3. Klik tombol "Tukar" untuk menggunakan XP</li>
          <li>4. Tunjukkan voucher ke mitra kami untuk menggunakan</li>
        </ul>
      </div>
    </div>
  </div>
